...
async function seedStudentProgress() {
  try {
    ...
    if (dbLevels.length >= 3) {
      ...
      console.log(`Levels 1-2 completed, level 3 in progress, levels ${dbLevels.length > 3 ? '4-' + dbLevels.length : ''} remain locked`);
    } else {
      console.warn(`Database Integration island has fewer than 3 levels (found ${dbLevels.length})`);
    }
