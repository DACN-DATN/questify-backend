...
import { Subjects } from '@datn242/questify-common';

// Initialize all event processors
export const initializeEventProcessors = () => {
  // Process Course Created event
  retryService.registerProcessor(Subjects.CourseCreated, async (data) => {
    try {
  ...
  retryService.registerProcessor(Subjects.IslandCreated, async (data) => {
    try {
  ...
  retryService.registerProcessor(Subjects.LevelCreated, async (data) => {
    try {
  ...
  retryService.registerProcessor(Subjects.PrerequisiteIslandCreated, async (data) => {
    try {
      ...
      console.log(`Successfully created prerequisite relationship: ${islandId} -> ${prerequisiteIslandId} on retry`);
      return true;
    } catch (error) {
  ...
  // Add processors for other event types as needed
  // ...
};